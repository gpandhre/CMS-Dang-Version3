...
<a class="new" href="newCom.php" style="text-decoration: none">
    <?php
        $countNew = mysqli_query($conn,"select count(*) as newComp from complaints where dept_head = 1 and collector = 0");
        if($countNew)
        {
...
<div class="dashboard">
    ...
    <?php
        $countAll = mysqli_query($conn,"select count(*) as allComp from complaints where dept_head = 1");
        if($countAll)
        {
    ...
    <div class="dash-elem">
        <?php
               $countPending = mysqli_query($conn,"select count(*) as penComp from complaints where dept_head = 1 and collector = 0 and approved = 0");
               if($countPending)
               {
